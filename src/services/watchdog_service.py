...
async def send_discord_notification(message: str, settings: Settings):
...
async def check_internet_connection(settings: Settings) -> bool:
...
async def watchdog_loop(device_registry: DeviceRegistry, settings: Settings):
    """The main loop for the watchdog service."""
    log.info("Starting watchdog service.")
    await send_discord_notification("Watchdog service started.", settings)
    ...
    while True:
        ...
        if await check_internet_connection(settings):
            if internet_down_since:
                downtime = (asyncio.get_event_loop().time() - internet_down_since)
                await send_discord_notification(f"Internet connection is back online after {downtime:.2f} seconds.", settings)
                internet_down_since = None
        else:
            if not internet_down_since:
                internet_down_since = asyncio.get_event_loop().time()
                await send_discord_notification("Internet connection is down.", settings)
        ...
        if now - last_chromecast_check > settings.CHROMECAST_DISCOVERY_INTERVAL:
            if not device_registry.get_all_devices():
                await send_discord_notification("No Chromecast devices detected. Starting a new discovery process.", settings)
        ...
        if now - last_chromecast_refresh > settings.CHROMECAST_REFRESH_INTERVAL:
            await send_discord_notification("Refreshing Chromecast devices.", settings)
